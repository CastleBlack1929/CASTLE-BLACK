  ...
  const readStoredRate = (key) => toNumber(localStorage.getItem(key));
    ...
    const startOscillation = (oscilarResumen) => {
      ...
      if (isOscYear2026) {
        ...
        setInterval(() => {
          const currentRateBase = currentRate || baseRate || DEFAULT_RATE_BY_YEAR.actual;
          if (ENABLE_RATE_OSCILLATION && Number.isFinite(currentRateBase)) {
            const rateFactor = 1 + ((Math.random() - 0.5) * 0.0005); // +/-0.025%
            currentRate = currentRateBase * rateFactor;
            histRateLive = currentRate;
            updateRateDisplay(currentRate);
          }
