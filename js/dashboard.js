...
let chartUtilidades = null;
  ...
  const OSCILLATION_FACTOR_RANGE = 0.002; // +/-0.1%
  const LOCK_CHART_SIZE = false;
...
const LOGO_BLACK_PATH = "img/logo-black.png";
  ...
  const readStoredRate = (key) => toNumber(localStorage.getItem(key));
    ...
    const prevPatrLBase = Number.isFinite(prevClosingPatrL)
      ? prevClosingPatrL
      : (Number.isFinite(baseRate) ? (prevClosingPatr || 0) * baseRate : null);
    ...
    // Oscilar patrimonio/resumen en un rango controlado de +/-0.1%.
    const startOscillation = (oscilarResumen) => {
      ...
      // Actual (2026): oscilar resumen cada 3s, sin mover la tasa.
      if (isOscYear2026) {
        ...
        setInterval(() => {
          ...
          const histFactor = baseAporteHist ? crFactor : (1 + ((Math.random() - 0.5) * OSCILLATION_FACTOR_RANGE));
    ...
    // Oscilacion activa segun condiciones del dashboard.
    ...
    if (graficoPatrimonio && graficoUtilidades && derived.monthly.length && typeof Chart !== "undefined") {
      ...
      const verticalLayoutMq = window.matchMedia("(max-width: 949px)");
      ...
      const resizeCharts = () => {
        if (chartPatrimonio) {
          chartPatrimonio.resize();
          chartPatrimonio.update("none");
        }
        if (chartUtilidades) {
          chartUtilidades.resize();
          chartUtilidades.update("none");
        }
      };
      ...
      const bindMediaResize = () => {
        const onLayoutChange = () => scheduleResize();
        if (wideLayoutMq.addEventListener) wideLayoutMq.addEventListener("change", onLayoutChange);
        else if (wideLayoutMq.addListener) wideLayoutMq.addListener(onLayoutChange);
        if (verticalLayoutMq.addEventListener) verticalLayoutMq.addEventListener("change", onLayoutChange);
        else if (verticalLayoutMq.addListener) verticalLayoutMq.addListener(onLayoutChange);
      };
