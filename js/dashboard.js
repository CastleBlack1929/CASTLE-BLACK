...
const LOGO_BLACK_PATH = "img/logo-black.png";
  ...
  try {
    ...
    const derived = computeDerived(userData.meses || {}, userData.patrimonioPrev || 0);
    derivedData = derived;
    ...
    // Solo para año actual (2026): usar patrimonio dic-2025 + movimientos 2026 + porcentaje Castle Black
    if (isActualYear) {
    ...
    if (utilidadTotal) utilidadTotal.textContent = formatMoney(utilidadUsd);
    setTrendClass(utilidad, utilidadUsd);
    setTrendClass(utilidadTotal, utilidadUsd);
    ...
    if (histBase) {
      if (aporteHist) aporteHist.textContent = formatMoney(histBase.totalAporte);
      if (patrimonioHist) patrimonioHist.textContent = formatMoney(histBase.patrimonioActual);
      if (utilidadRHist) utilidadRHist.textContent = formatMoney(histBase.utilidadActual);
      if (utilidadHist) utilidadHist.textContent = formatMoney(histBase.utilidadActual);
    ...
    const aporteBaseLInitial = isActualYear
      ? (Number.isFinite(prevClosingPatrL) && prevClosingPatrL > 0
          ? prevClosingPatrL + sumMovCop
          : (toNumber(userData.aporteL) || 0) + sumMovCop)
    ...
    applyPesos = (rate, patrUsdOverride = null, utilUsdOverride = null) => {
      ...
      const aporteCop = Number.isFinite(aporteBaseL) ? aporteBaseL : usdAporte * rate;
    ...
    if (histBase && aporteHistL && patrimonioHistL && utilidadRHistL && utilidadHistL && crcmntHistL) {
      ...
      utilidadHistL.textContent = formatMoney(utilCopHist);
    ...
    if (tablaMeses) {
      ...
      if (derived.monthly.length) {
        derived.monthly.forEach(({ mes, aporte, patrimonio: patrVal, margen, g_p }) => {
          ...
          const isJanCurrent = isActualYear && Number(displayYear) === 2026 && mes === "enero";
          const patrToShow = isJanCurrent ? patrimonioCalc : patrVal;
