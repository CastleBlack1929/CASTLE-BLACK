...
const LOGO_BLACK_PATH = "img/logo-black.png";
  ...
  try {
    ...
    if (histBase) {
      if (aporteHist) aporteHist.textContent = formatNumber(histBase.totalAporte);
      if (patrimonioHist) patrimonioHist.textContent = formatNumber(histBase.patrimonioActual);
      if (utilidadRHist) utilidadRHist.textContent = formatNumber(histBase.utilidadActual);
      if (utilidadHist) utilidadHist.textContent = formatNumber(histBase.utilidadActual);
      if (crcmntHist) crcmntHist.textContent = formatPercent(histBase.crcmntActual);
      setTrendClass(utilidadRHist, histBase.utilidadActual);
      setTrendClass(utilidadHist, histBase.utilidadActual);
      setTrendClass(crcmntHist, histBase.crcmntActual);
    }
    ...
    if (histBase && aporteHistL && patrimonioHistL && utilidadRHistL && utilidadHistL && crcmntHistL) {
      ...
      const utilCopHist = (histBase.utilidadActual || 0) * LATEST_HISTORICAL_RATE;
      ...
      if (utilidadRHist) utilidadRHist.textContent = formatNumber(histBase.utilidadActual);
      if (utilidadHist) utilidadHist.textContent = formatNumber(histBase.utilidadActual);
      aporteHistL.textContent = formatNumber(aporteCopHist);
      patrimonioHistL.textContent = formatNumber(patrCopHist);
      utilidadHistL.textContent = formatNumber(utilCopHist);
      utilidadRHistL.textContent = formatNumber(utilRCopHist);
      crcmntHistL.textContent = formatPercent(crcmntHistLCur);
      setTrendClass(utilidadHistL, utilCopHist);
      ...
      setInterval(() => {
        ...
        if (histBase) {
          ...
          if (utilidadRHist) {
            ...
            utilidadRHist.textContent = formatNumber(utilHistOsc);
            if (utilidadRHistArrow) {
              utilidadRHistArrow.textContent = utilHistOsc > prevUR ? "▲" : (utilHistOsc < prevUR ? "▼" : "—");
            }
          }
          if (utilidadHist) {
            const prevU = toNumber(utilidadHist.textContent) ?? utilCalcBase;
            utilidadHist.textContent = formatNumber(utilHistOsc);
            if (utilidadHistArrow) {
              utilidadHistArrow.textContent = utilHistOsc > prevU ? "▲" : (utilHistOsc < prevU ? "▼" : "—");
            }
          }
          if (crcmntHist) {
            crcmntHist.textContent = formatPercent(nuevoCrcmnt);
          }
          // COP histórico oscilando con tasa más reciente
          const aporteCopHist = aporteBaseHist * latestRate;
          const patrCopHist = patrHistOsc * latestRate;
          const utilCopHist = utilHistOsc * latestRate;
          const utilRCopHist = utilCopHist;
          const crcmntHistLCur = aporteCopHist !== 0 ? (utilCopHist / Math.abs(aporteCopHist)) * 100 : 0;
          if (aporteHistL) aporteHistL.textContent = formatNumber(aporteCopHist);
          if (patrimonioHistL) patrimonioHistL.textContent = formatNumber(patrCopHist);
          if (utilidadHistL) {
            utilidadHistL.textContent = formatNumber(utilCopHist);
            setTrendClass(utilidadHistL, utilCopHist);
          }
        ...
        if (oscilarResumen && Number.isFinite(rateToUse)) {
        ...
        try {
          ...
          addTable(
            ...
            [
              ...
              ["Utilidad", fmtMoney(histUtilUsd), fmtMoney(histUtilUsd * latestHistoricalRate)]
            ]
          );
