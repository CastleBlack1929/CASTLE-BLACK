        ...
        try {
          ...
          if (logoUrl) {
            try {
              ...
              doc.addImage(logoUrl, "PNG", logoX, y - 2, logoSize, logoSize);
            } catch (e) {
          ...
          y += 4.5;
          doc.text("Bogotá, Colombia | Atención: L-V 8:00 - 18:00 (GMT-5)", marginX, y);
          y += 7;
          doc.setFontSize(10);
          doc.text("Informe con los datos visibles en tu tablero: aportes, patrimonio, utilidades, honorarios y movimientos", marginX, y);
          y += 4.5;
          doc.text("Los valores en cero indican meses no cerrados o sin datos cargados.", marginX, y);
          y += 6;
          addParagraph("Este estado de cuenta resume tu posición con Castle Black para el periodo seleccionado. Incluimos aportes, patrimonio y utilidades en USD y COP, más los honorarios aplicados y los movimientos registrados. El documento es un reflejo exacto de lo visible en tu dashboard y está pensado para compartir con tu auditor o contabilidad.");
          addParagraph("Metodología: cifras en USD calculadas con base en tus aportes y patrimonio acumulado; cifras en COP convertidas con la tasa vigente del tablero. Las utilidades consideran ganancias/pérdidas y comisiones. Las secciones históricas muestran la evolución total desde tu fecha de ingreso.");

          addLine(`Cliente: ${safeText(nombreCliente)}`);
          const cleanId = safeText(idClienteHeader).replace(/^ID:\\s*/i, "") || "N/D";
          addLine(`ID: ${cleanId}`);
          addLine(`Nivel: ${safeText(nivelText)}`);
          ...
          addTable(
            "Resumen de cuenta",
            "Aportes, patrimonio, utilidades y crecimiento en USD y COP tal como se muestran en tu dashboard.",
            ["", "USD", "COP"],
            [
          ...
          addParagraph("Interpretación: Patrimonio y utilidad en USD, crecimiento porcentual sobre aportes acumulados. Conversión a COP usando la tasa vigente al cierre del periodo visible.");

          y += 4;
          addLine("Estado de cuenta total (histórico)", 12, 8, "bold");
          addParagraph("Como consecuencia de los aportes y movimientos realizados durante el tiempo que has mantenido tu relación con nosotros, este bloque refleja el margen de utilidad acumulado histórico de tu inversión y el crecimiento real de tu capital, considerando utilidades, retiros y comisiones.");
          addTable(
            "Totales históricos",
            "Cifras acumuladas desde tu fecha de ingreso. El crecimiento refleja el rendimiento sobre el total aportado.",
            ["Concepto", "USD", "COP"],
            [
              ["Aporte", safeText(aporteHist), safeText(aporteHistL)],
              ["Patrimonio", safeText(patrimonioHist), safeText(patrimonioHistL)],
              ["Crecimiento", safeText(crcmntHist), safeText(crcmntHistL)],
              ["Utilidad R", safeText(utilidadRHist), safeText(utilidadRHistL)],
              ["Utilidad", safeText(utilidadHist), safeText(utilidadHistL)]
            ]
          );
          ...
          addTable(
            ...
            "Aporte y patrimonio al cierre de cada mes, margen porcentual sobre el patrimonio previo y ganancia/pérdida mensual. Usa estos datos para validar tu curva de crecimiento y comparar periodos.",
          ...
          addTable(
            ...
            "Cálculo trimestral según utilidad acumulada en cada tramo y la tarifa correspondiente al escalón alcanzado. Incluye los cobros de administración sobre tu utilidad generada.",
            ["Trimestre", "Tarifa", "Comisión", "Valor"],
            honorariosRows
          );
          addLine(`Total honorarios: ${formatNumber(honorariosTotalUsd)}`, 10, 6, "bold");
          ...
          addTable(
            "Movimientos",
            "Consignaciones y retiros registrados en el año seleccionado, con tasa de cambio y valor en COP para cada operación. Usa este cuadro para contrastar tus recibos y estados bancarios.",
            ["Recibo", "Fecha", "Cantidad", "Tipo", "Tasa", "Cambio"],
            movimientosRows
          );

          y += 2;
          addLine("Contacto", 12, 8, "bold");
